...
GitHub Actions runs the shared release validation script for pull requests, pushes to `main`, and `v*` tag validation. The script validates the SwiftPM manifest, SwiftPM build/test, Release safety gates, documentation links, release content audit, DocC documentation, the package iOS Simulator build, both sample app builds, and source archive creation before a GitHub Release is published.
