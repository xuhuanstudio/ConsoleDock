...
该脚本会先确认工作区干净，再验证 SwiftPM manifest、package identity、Swift Package Index 元数据、Objective-C API surface、UI accessibility identifiers、Swift 格式、构建、测试、Release safety gates、文档链接、开源治理元数据、公开发布内容审计、DocC、iOS package build、Swift/Objective-C 示例 App 构建、source archive 生成、source archive 内容和 source archive 独立构建/测试。设置 `CONSOLEDOCK_RUN_UI_SMOKE=1` 可以额外运行 Swift 示例 App 的 iOS Simulator UI smoke test。
