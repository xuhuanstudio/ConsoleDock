...
- 带 session 内稳定 ID 和 redacted/truncated 标记的本地内存日志存储；
- 默认敏感字段脱敏；
- UIKit 浮动按钮和日志面板；
...
该脚本会先确认工作区干净，再验证 SwiftPM manifest、package identity、Swift Package Index 元数据、Objective-C API surface、Swift 格式、构建、测试、Release safety gates、文档链接、开源治理元数据、公开发布内容审计、DocC、iOS package build、Swift/Objective-C 示例 App 构建、source archive 生成、source archive 内容和 source archive 独立构建/测试。
