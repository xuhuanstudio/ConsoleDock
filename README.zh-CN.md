...
ConsoleDock `v0.5.0` 是当前 source-first Swift Package Manager 公开预览版本，包含：
...
- Debug Actions，用于注册 App 主动提供的本地测试快捷动作，并支持 disabled/destructive 元数据；
...
如果需要可靠完整的 App 内日志展示，推荐使用 ConsoleDock 的显式 API，或者在已有 logger 中增加 sink/appender 转发。`v0.5.0` 提供的 `ConsoleDock.LogForwarder` / `CDKLogForwarder` 就是为这个迁移路径准备的轻量工具。
...
ConsoleDock 只展示和触发 App 注册的动作；它不会自动发现页面、接管路由、绕过业务权限、远程下发命令，也不是自动化测试平台。`isEnabled` 可用于展示暂不可用动作，`.destructive` 可用于清理本地调试数据这类危险动作的 UI 提示。
