...
The verifier checks the GitHub repository, remote tag, GitHub Release state, release-note boundaries and matching validation link, `Release Validation` workflow, a clean external SwiftPM consumer build, and Swift Package Index package/DocC pages when `--check-spi` is supplied.
