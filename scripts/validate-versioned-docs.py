...
REQUIRED_SNIPPETS = {
    "README.md": [
        "ConsoleDock `v0.3.2` is the current source-first Swift Package Manager preview release.",
        "Use the latest release tag from GitHub Releases. `v0.3.2` includes Debug Actions, log detail, explicit visible/all sharing, runtime diagnostics, and release-validation hardening.",
        "Runtime diagnostics are available in `v0.2.0` and later.",
        "Debug Actions are available in `v0.3.0` and later.",
    ],
    "README.zh-CN.md": [
        "ConsoleDock `v0.3.2` 是当前 source-first Swift Package Manager 公开预览版本",
...
def self_test() -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="consoledock-versioned-docs-self-test-") as raw_directory:
        ...
        if validate(root):
            errors.append("validate should accept docs with released v0.3.2 guidance")
