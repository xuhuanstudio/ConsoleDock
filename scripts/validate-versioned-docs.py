...
REQUIRED_SNIPPETS = {
    ...
    "README.zh-CN.md": [
        ...
        "通过 Swift Package Manager 添加公开仓库地址，并选择 GitHub Releases 中最新的 release tag。`v0.12.0` 已包含 Local Session Archive 显式保存/查看/删除、内置 Session Timeline、Logs 本地结构化查询、next/previous visible error jump、local Debug Action execution history、action form session-only 最近参数值复用、reproduction timeline issue reports、临时 `.txt` issue-report 分享、parameterized Debug Actions、App Context、可配置 floating trigger、Logs Jump、Actions 搜索、logger forwarders、Test Session Reports、manual markers、Debug Actions、日志详情、visible/all/issue-report 分享和复制、runtime diagnostics、当前 iOS Simulator 截图、文档图片校验、视觉 QA 指南和当前 release validation 加固：",
...
def self_test() -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="consoledock-versioned-docs-self-test-") as raw_directory:
        ...
        if validate(root):
            errors.append("validate should accept docs with released v0.12.0 guidance")
